...
public class AppController {
	...
	public ResponseEntity addUser(@RequestBody AppDomain appDomain) {
		if (appService.validate(appDomain.getEmail()) && appService.validateLetters(appDomain.getName())) {
		appService.add(appDomain);
		return new ResponseEntity<String>("Added successfully", HttpStatus.OK);
		}
		else
			return new ResponseEntity<String>("Invalid email Or Name.", HttpStatus.EXPECTATION_FAILED);
